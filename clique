...
let dfsClique (Gr arcs) s k clique =
  let rec visit visited clique = function 
      [] -> raise CliqueNotFound
    | n::nodes ->
        if (List.length clique) == k then clique
...
(* cliqueOne: 'a graph -> 'a list -> 'a list *)
let cliqueOne (Gr arcs) clique =
...
let rec searchClique graph n = function
  ...
  | clique::cliques -> 
      try 
        dfsClique graph clique n [clique]
      with CliqueNotFound ->
...
let clique (Gr arcs) n =
  ...
  else searchClique (Gr arcs) n (cliqueOne (Gr arcs) []);;
